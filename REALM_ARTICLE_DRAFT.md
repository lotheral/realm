...
**Suvar Ergun** · 2026 · v0.25.0 · MIT License · [`github.com/lotheral/realm`](https://github.com/lotheral/realm)
...
**Benchmark.** 22 historical events across 7 countries (US, GB, DE, FR, TR, FI, SE), each with a documented before/after opinion measurement from a named pollster or index, an event summary written as outcome-free news copy, a country-scoped target population, and a mechanism tag: 9 *rally* (negative event → leader approval rises), 5 *approval_drop*, 6 *policy_shift* (including threat-to-status-quo cases), 2 *confidence_index*. 22/22 events' numbers are verified against sources (the verification pass corrected 5 authored values and replaced one unverifiable series; a later pass caught the Swedish NATO baseline recorded as Demoskop's *against* share instead of its *for* share — a recorded lesson, twice, that authored numbers are candidates, never data). Rally events are included *because* they are hard: excluding them would be calibration theater.
...
| Signed Spearman ρ (predicted vs observed) | **−0.506** |
...
3. **Magnitude quantization:** outputs cluster near 0, ±20–29, and ±42–46pp — artifacts of the perturbation floor/cap and affected-population ratio — leaving no usable magnitude ranking. We subsequently removed the quantization (a smooth tanh map replaces the floor/cap clamp: 15 distinct parser scores now yield 14 distinct magnitudes instead of 6) and re-ran the benchmark: directional accuracy is unchanged and magnitude ρ moves only from −0.124 to −0.066 — the artifact is gone, and the lexicon scores still carry no magnitude information. The quantization explained the *shape* of the outputs, not the absence of signal.
...
| Categories | 9 |
| Benchmark | Study A: 22 events, 7 countries, 22/22 verified, blinded harness (`scripts/run_study_a.py`) |
| Diary | Study B: append-only forward registry (`scripts/diary.py`) |
| Validation | BF 8/8 PASS · Study A DA 4/22 (negative, published; corrected post-audit) · 1026 automated tests · CI |
| License | MIT |
